...
void OpenGLContext::DrawIndexedUP(const void *vdata, int vertexCount, const void *idata, int indexCount) {
	...
	ApplySamplers();
	renderManager_.DrawIndexed(curPipeline_->inputLayout->inputLayout_.get(), vbuf, voffset, ibuf, ioffset, curPipeline_->prim, indexCount, GL_UNSIGNED_SHORT, 1);
}
...
void OpenGLContext::DrawIndexedClippedBatchUP(const void *vdata, int vertexCount, const void *idata, int indexCount, Slice<ClippedDraw> draws, const void *ub, size_t ubSize) {
	...
	for (auto &draw : draws) {
		...
		if (draw.bindTexture) {
			...
			renderManager_.BindFramebufferAsTexture(((OpenGLFramebuffer*)draw.bindFramebufferAsTex)->framebuffer_.get(), 0, GL_COLOR_BUFFER_BIT);
		}
		GLRect2D scissor;
		...
		renderManager_.DrawIndexed(curPipeline_->inputLayout->inputLayout_.get(), vbuf, voffset, ibuf, ioffset + draw.indexOffset * indexSize, curPipeline_->prim, draw.indexCount, GL_UNSIGNED_SHORT, 1);
	}
}
...
void OpenGLContext::CopyFramebufferImage(Framebuffer *fbsrc, int srcLevel, int srcX, int srcY, int srcZ, Framebuffer *fbdst, int dstLevel, int dstX, int dstY, int dstZ, int width, int height, int depth, Aspect aspects, const char *tag) {
	...
	renderManager_.CopyFramebuffer(src->framebuffer_.get(), GLRect2D{ srcX, srcY, width, height }, dst->framebuffer_.get(), GLOffset2D{ dstX, dstY }, glAspect, tag);
}
...
void OpenGLContext::BindFramebufferAsTexture(Framebuffer *fbo, int binding, Aspect aspects, int layer) {
	...
	renderManager_.BindFramebufferAsTexture(fb->framebuffer_.get(), binding, glAspect);
}
