	...
	if (deviceFormat_ && !ValidateFormat(deviceFormat_)) {
		...
		if (hr == S_OK) {
			...
			if (ValidateFormat(closest)) {
				...
				ERROR_LOG_REPORT_ONCE(badfallbackclosest, SCEAUDIO, "WASAPI fallback and closest unsupported (fmt=%04x/%s)", closest->Format.wFormatTag, ConvertWStringToUTF8(guid).c_str());
				CoTaskMemFree(closest);
				return false;
			}
		} else {
			...
			if (hr != AUDCLNT_E_DEVICE_INVALIDATED && hr != AUDCLNT_E_SERVICE_NOT_RUNNING)
				ERROR_LOG_REPORT_ONCE(badfallback, SCEAUDIO, "WASAPI fallback format was unsupported (%08lx)", hr);
			return false;
		}
	}
