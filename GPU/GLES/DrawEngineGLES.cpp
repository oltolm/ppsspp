...
void DrawEngineGLES::BeginFrame() {
	...
	render_->BeginPushBuffer(frameData.pushIndex.get());
	render_->BeginPushBuffer(frameData.pushVertex.get());
...
void DrawEngineGLES::EndFrame() {
	FrameData &frameData = frameData_[render_->GetCurFrame()];
	render_->EndPushBuffer(frameData.pushIndex.get());
	render_->EndPushBuffer(frameData.pushVertex.get());
	...
	if (vshader->UseHWTransform()) {
		...
		if (result.action == SW_DRAW_PRIMITIVES) {
			if (result.drawIndexed) {
				...
				render_->Draw(
					softwareInputLayout_.get(), vertexBuffer, vertexBufferOffset, glprim[prim], 0, result.drawNumTrans);
			}
